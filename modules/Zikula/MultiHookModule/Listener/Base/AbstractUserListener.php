...
abstract class AbstractUserListener implements EventSubscriberInterface
{
    ...
    public function delete(GenericEvent $event): void
    {
        ...
        $repo = $this->entityFactory->getRepository('entry');
        // set creator to admin (UsersConstant::USER_ID_ADMIN) for all entries created by this user
        $repo->updateCreator($userId, UsersConstant::USER_ID_ADMIN, $this->translator, $this->logger, $this->currentUserApi);
        
        // set last editor to admin (UsersConstant::USER_ID_ADMIN) for all entries updated by this user
        $repo->updateLastEditor($userId, UsersConstant::USER_ID_ADMIN, $this->translator, $this->logger, $this->currentUserApi);
        
        $logArgs = ['app' => 'ZikulaMultiHookModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'entries'];
