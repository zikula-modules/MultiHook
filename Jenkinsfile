    ...
    stages {
        ...
        stage('Create release packages') {
            steps {
                ...
                sh 'cd releaseWork/modules/Zikula/MultiHookModule && ../../../../build/composer.phar install --no-dev --optimize-autoloader && cd ../../../../'
                sh 'cp -R releaseWork/app/Resources/ZikulaMultiHookModule/* releaseWork/modules/Zikula/MultiHookModule/Resources/'
                sh 'rm -rf releaseWork/app'


                sh 'cd releaseWork && zip -D -r ../release/MultiHook.zip . && cd ..'
                sh 'cd releaseWork && tar cfvz ../release/MultiHook.tar.gz . && cd ..'
